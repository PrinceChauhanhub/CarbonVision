...
if tab == "\ud83d\udc64 Personal":
    ...
    st.session_state.sex = st.selectbox("Sex", ["Select...", "male", "female"])
    st.session_state.social_activity = st.selectbox("Social Activity", ["Select...", "never", "sometimes", "often"])
    st.session_state.diet = st.selectbox("Diet", ["Select...", "omnivore", "vegetarian", "pescatarian", "vegan"])

elif tab == "\ud83d\ude97 Travel":
    st.session_state.vehicle_type = st.selectbox("Vehicle Type", ["Select...", "diesel", "walk/bicycle/publicvehicle", "petrol", "hybrid", "electric", "lpg"])
    st.session_state.transport = st.selectbox("Transportation", ["Select...", "private", "public", "walk/bicycle"])
    st.session_state.vehicle_monthly_distance_km = st.slider("Monthly vehicle distance (Km)", 0, 9999)
    st.session_state.air_travel_frequency = st.selectbox("Air travel frequency", ["Select...", "never", "rarely", "frequently", "very frequently"])

elif tab == "\ud83d\uddd1 Waste":
    st.session_state.waste_bag_size = st.selectbox("Waste bag size", ["Select...", "small", "medium", "large", "extra large"])
    st.session_state.waste_bag_weekly_count = st.number_input("Weekly waste bag count", 1, 7)
    st.session_state.recycling = st.multiselect("Recycling materials", ['Metal', 'Paper', 'Plastic', 'Glass', 'Electronics'])

elif tab == "\u26a1 Energy":
    st.session_state.heating_energy_source = st.selectbox("Heating power source", ["Select...", "coal", "natural gas", "electricity", "wood"])
    st.session_state.cooking_with = st.multiselect("Cooking methods", ['Grill', 'Airfryer', 'Stove', 'Oven', 'Microwave'])
    st.session_state.energy_efficiency = st.selectbox("Energy-efficient devices?", ["Select...", "Yes", "No"])
    st.session_state.tv_pc_daily_hours = st.slider("Daily PC/TV usage (hours)", 0, 24)
    st.session_state.internet_daily_hours = st.slider("Daily internet usage (hours)", 0, 24)

elif tab == "\ud83d\udc8a Consumption":
    st.session_state.shower_frequency = st.selectbox("Shower frequency", ["Select...", "daily", "twice a day", "less frequently", "more frequently"])
    st.session_state.monthly_grocery_bill = st.slider("Monthly grocery bill ($)", 50, 299)
    st.session_state.new_clothes_monthly = st.slider("New clothes bought monthly", 0, 50)
